...
public class Sagot {
    ...
    public Sagot(int max_error, int max_motif_wildcard, int max_deletion, int max_insertion, int quorum1, int quorum2,
                 ...
                 GeneralizedSuffixTree data_t, Trie motif_trie, boolean count_by_keys, Utils utils) throws Exception {
        ...
        findMotifs(motif_trie.getRoot());
    }

    /**
     * Calls the recursive function spellMotifs
     * @param enum_node a node in the motif tree, the motif tree traversal begins in this node
     */
    private void findMotifs(MotifNode enum_node) {

        data_tree.computeCount();
        total_chars_in_data = ((OccurrenceNode) data_tree.getRoot()).getCount_by_indexes();

        OccurrenceNode words_root = (OccurrenceNode) data_tree.getRoot();
        //occurrence of empty string
        Occurrence empty_occ = new Occurrence(words_root, null, -1, 0, 0);

        enum_node.addOcc(empty_occ, max_insertion);

        spellMotifs(enum_node, "", 0, 0);
    }
     ...
     *
     * @param enum_node node in the enumeration tree that represents the current motif
     * @param motif     represents concatenation of edged from root to enum_node
     * @param motif_length
     ...
     */
    private int spellMotifs(MotifNode enum_node, String motif, int motif_length, int motif_wildcard_count) {
        if (motif_wildcard_count < max_motif_wildcard && enum_node.getType().equals("enumeration")) {
            //add to enum_node an edge with "_", pointing to a new node that will save the occurrences
            addWildcardEdge(enum_node, true);
        }

        ArrayList<Occurrence> Occ = enum_node.getOccs();

        HashMap<Integer, MotifNode> target_nodes = enum_node.getTarget_nodes();
        ...
        for (Map.Entry<Integer, MotifNode> entry : target_nodes.entrySet()) {
            ...
            if (alpha!=wildcard_char) {
                num_of_diff_occ = extendMotif(alpha, motif_wildcard_count, motif, target_node, enum_node, Occ, motif_length);

                if (num_of_diff_occ > max_num_of_diff_occ) {
                    max_num_of_diff_occ = num_of_diff_occ;
                }
                //For memory saving, remove pointer to target node
                enum_node.addTargetNode(alpha, null);
            }
        }

        //handle wild card edge
        if (enum_node.getType().equals("motif") || motif_wildcard_count < max_motif_wildcard) {
            target_node = enum_node.getTargetNode(wildcard_char);
            if (target_node != null) {
                num_of_diff_occ = extendMotif(wildcard_char, motif_wildcard_count + 1, motif, target_node, enum_node, Occ, motif_length);
                if (num_of_diff_occ > max_num_of_diff_occ) {
                    max_num_of_diff_occ = num_of_diff_occ;
                }
            }
     ...
     * @param enum_node            node of motif
     * @param Occ                  the occurrences of motif
     * @param motif_length
     * @return num of different occurrences of extended motif
     */
    private int extendMotif(int alpha, int motif_wildcard_count, String motif, MotifNode target_node,
                            MotifNode enum_node, ArrayList<Occurrence> Occ, int motif_length) {
        ...
        if (motif_wildcard_count > 0 && alpha!=wildcard_char) {
            ...
            enum_node.addTargetNode(alpha, extended_motif_node);
        }
        ...
        if (exact_occs_count >= q1 && diff_occs_count >= q2) {
            int ret = spellMotifs(extended_motif_node, extended_motif, extended_motif_length, motif_wildcard_count);
